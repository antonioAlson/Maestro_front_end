...
const JQL = \`((project = MANTA AND "fábrica de manta[dropdown]" = COMTEC AND status IN ("A Produzir", "Liberado Engenharia","Em Produção", "Produzido")) OR (project = TENSYLON AND "fábrica de tensylon[dropdown]" = COMTEC AND status IN ("A Produzir", "Liberado Engenharia", "Aguardando Acabamento", "Aguardando Autoclave", "Aguardando Corte", "Aguardando montagem","🔴RECEBIDO NÃO LIBERADO")))\`;
...
async function salvarOuAtualizar(issue) {
  const sql = \`
    INSERT INTO maestro.jira_cards (
      key, tipo, resumo, status, situacao, veiculo, previsao, last_updated_at
    )
  ...
  const values = [
    issue.key, issue.tipo, issue.resumo, issue.status, issue.situacao,
    issue.veiculo, issue.previsao || null,
  ];
  await pool.query(sql, values);
}
...
async function processar() {
  ...
  do {
    ...
    for (const issue of issues) {
      ...
      await salvarOuAtualizar({
        key, tipo, resumo, status, situacao, veiculo, previsao: previsaoRaw,
      });

      const novaData = proximoDiaUtil();
      console.log(key, tipo, "previsão calculada:", novaData);
      // Atualização da previsão no Jira permanece comentada (igual ao legado).
      total++;
    }
...
const LEGACY_JOBS = [
  ...
  {
    name: 'update_comtec_cards',
    description: 'Atualiza datas/situações de cards COMTEC (MANTA + TENSYLON) e calcula próximo dia útil.',
    cron_expression: '0 8 * * *',
    code: UPDATE_COMTEC_CODE,
  },
];
